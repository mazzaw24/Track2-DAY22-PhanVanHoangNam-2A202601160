...
# Windows terminals may default to a legacy code page (for example cp1258),
# which cannot encode every precomposed Vietnamese character.  Force UTF-8
# for both normal output and tracebacks, including when output is piped to tee.
...
PROMPT_V1_NAME = "phan-van-hoang-nam-rag-v1"
PROMPT_V2_NAME = "phan-van-hoang-nam-rag-v2"
...
SYSTEM_V1 = (
    "Bạn là trợ lý AI hữu ích. Chỉ sử dụng context được cung cấp để trả lời. "
    "Trả lời ngắn gọn trong 2-4 câu và không thêm thông tin không có trong context. "
    "Nếu context không đủ, hãy nói rõ bạn không biết.\n\nContext:\n{context}"
)
...
SYSTEM_V2 = (
    "Bạn là chuyên gia AI. Hãy đọc kỹ context, xác định các dữ kiện liên quan và "
    "trả lời rõ ràng, có cấu trúc trong 3-5 câu. Chỉ đưa ra các khẳng định được "
...
def push_prompts_to_hub(client: Client):
    ...
    try:
        url = client.push_prompt(
            PROMPT_V1_NAME, object=PROMPT_V1, description="V1 – ngắn gọn"
        )
    ...
    try:
        url = client.push_prompt(
            PROMPT_V2_NAME, object=PROMPT_V2, description="V2 – có cấu trúc"
        )
